...
extrap_folder = "/cluster/work/math/climate-downscaling/cordex-data/cordex-ALPS-allyear/cmip6/test/extrapolation"
output_folder = "../plotting_data/results_matrices/cmip6/"
...
def compute_metrics(solar, wind, time_dim):
    ...
    combined = ((solar["rsds"] <= rsds_thresh) & (wind["sfcWind"] <= wind_thresh)).astype("float32")
    return {
        "combined_mean": combined.mean(dim=time_dim),
        "solar_below_mean": solar["rsds"].where(solar["rsds"] <= rsds_thresh).mean(dim=time_dim),
        "wind_below_mean": wind["sfcWind"].where(wind["sfcWind"] <= wind_thresh).mean(dim=time_dim)
    }
...
for file_path in files[1:]:
    ...
    # --- Identify model and matching forcing files ---
    model_name = base.split("_")[4]
    print(model_name)
    solar_file = [f for f in solar_files if model_name in f][0]
    wind_file = glob.glob(os.path.join(extrap_folder, f"*sfcWind*{model_name}*EUROPE*.nc"))[0]

    # --- Load GCM data ---
    solar_data = xr.open_dataset(solar_file).sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    wind_data  = xr.open_dataset(wind_file).sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))

    # --- Load RCM data ---
    data_pred  = xr.open_dataset(file_path, group='prediction', chunks={'ensemble': 2, 'time': 365, 'y': 128, 'x': 128})
