...
for file_path in files:
    ...
    rsds_bin = rsds_bin.assign_coords(
        season=("time", season_labels[0 : rsds_bin.shape[1]])
    )
    wind_bin = wind_bin.assign_coords(
        season=("time", season_labels[0 : wind_bin.shape[1]])
    )
    ...
    np.save(
        f"../plotting_data/corrdiff/per_file_id011/solar/{filename}.npy",
        rsds_counts_np,
    )
    np.save(
        f"../plotting_data/corrdiff/per_file_id011/wind/{filename}.npy",
        wind_counts_np,
    )
...
# files = list of your NetCDF files
all_season_counts = {}  # dictionary to store results per file
...
for file_path in files:
    ...
    filename = file_path.split("/")[-1].rsplit(".", 1)[0]
    np.save(
        f"../plotting_data/corrdiff/per_file_id011/solar/{filename}.npy", rsds_counts_np
    )
    np.save(
        f"../plotting_data/corrdiff/per_file_id011/wind/{filename}.npy", wind_counts_np
    )
    ...
    # Folder containing your files
data_folder = "/cluster/work/math/climate-downscaling/cordex-data/cordex-ALPS-allyear/benchmarks/corrdiff/"
file_pattern = "*id-020*2090-2099*.nc"
...
for file_path in files:
    ...
    # 7. Save separately
    filename = file_path.split("/")[-1].rsplit(".", 1)[0]
    np.save(f"../plotting_data/orig_RCM_solar/{filename}.npy", rsds_counts_np)
    np.save(f"../plotting_data/orig_RCM_wind/{filename}.npy", wind_counts_np)
